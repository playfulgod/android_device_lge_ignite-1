...
struct alsa_device_t {
    ...
    status_t (*standby)(alsa_handle_t *); //By Veena
    status_t (*voicevolume)(float);
    status_t (*set)(const String8&);
    status_t (*popAttenu)(int);	// 20100604 junyeop.kim@lge.com, No pop noise set(amp on delay) [START_LGE]
    status_t (*setInput)(int);	//[LG_FW_P970_MERGE] - jungsoo1221.lee /* 20110219 jiwon.seo@lge.com for input gain seperation */
};
...
public:
    ...
    status_t 		    setVoiceVolume(int value); 		/* [2011.02.21]jung.chanmin@lge.com - add the voice volume gain */
	status_t 			setVoiceCurrentDev(int cur_dev);	// 20101011 junyeop.kim@lge.com set the voice device[START_LGE]
 //[LG_FW_AUDIO_TTY Start] - jungsoo1221.lee
	status_t			setTtyMode(int mode);
	int 				getTtyMode();
 //[LG_FW_AUDIO_TTY End]
#ifdef AUDIO_MODEM_TI
...
public:
    ...
    virtual status_t		popNoise_attenu(int enable);        // 20100604 junyeop.kim@lge.com, No pop noise set(amp on delay) [START_LGE]
...
#if 1			
	virtual int	setLoopbackMode(int mode);
#if 0	
	virtual int	setVoccalParam(int voc_codec, int voccal_param_type, int get_flag, int param_val);
	virtual int	setVoccalIIRParam(int voc_codec, int voccal_iir_param_type,  int get_flag, int param_val);
	virtual int	setNextEcParam(int ec_mode, int ec_param_type,  int get_flag, int param_val);
	virtual int	setRxVolume(int device, int method, uint8_t idx,  int get_flag, int value);
	virtual int	setDtmfVolume(int device, int method, uint8_t idx,  int get_flag, int value);
	virtual int	setPadValue(int device, int method, uint8_t idx,  int get_flag, int value);
	virtual int	setAmpGain(int voc_codec, int gain_type,  int get_flag, int value);
	virtual int	writeEFS(void);
	virtual int	setMicAmpGain(int voc_codec, int channel,  int get_flag, int value);
	virtual int	writeMEM(void);
	virtual int	setVoiceClarity(int mode);
	virtual int	setPowerOff(void);
	virtual int	setFMRadioMultiSound(void);
#endif
#endif
//#endif
    // mic mute
 ...
 //[LG_FW_AUDIO_TTY Start] - jungsoo1221.lee
	virtual status_t	setTtyMode(int mode);
	virtual status_t	getTtyMode(int* mode);
 //[LG_FW_AUDIO_TTY End] 
...
protected:
    ...
    friend class ALSAControl; /* [2011.02.21]jung.chanmin@lge.com - add the voice volume gain */
    ALSAMixer *         mMixer;

    ALSAControl  *mControl; /* [2011.02.21]jung.chanmin@lge.com - add the voice volume gain */

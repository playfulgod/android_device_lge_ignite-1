...
#if 0	//[LG_FW_P970_MERGE Start] - jungsoo1221.lee  //junyeop.kim@lge.com, original code
    if (rate && *rate > 0) {
...
#else
	if (rate && *rate > 0) {
        if (*rate == 44100 || *rate == 8000 || *rate == 16000 || *rate == 11025 || *rate == 22050)
...
status_t ALSAStreamOps::setParameters(const String8& keyValuePairs)
{
    ...
    String8 key_input = String8("input_source"); //[LG_FW_P970_MERGE]-jungsoo1221.lee /* 20110219 jiwon.seo@lge.com for input gain seperation */
...
/* 20110219 jiwon.seo@lge.com for input gain seperation [START] */
    if (param.getInt(key_input, device) == NO_ERROR) {
       LOGD("setParameters() key_input = %s, device = %d",key_input.string(), (uint32_t)device);
       mParent->mALSADevice->setInput(device);
       param.remove(key_input);
    }
/* 20110219 jiwon.seo@lge.com for input gain seperation [END] */
...
#else
    //return (1024 * 8); //bytes;	 
//	 return (1024 * 4); //bytes; //minyoung1.kim@lge.com myoung_recording 

//2011.05.16 minyoun1.kim@lge.com 
	LOGD("ALSAStreamOps::bytes %d", bytes);

	if(mHandle->devices & AudioSystem::DEVICE_OUT_ALL)	//shinuk.lee_to_separate_audio_capture
	{
		LOGD("ALSAStreamOps::DEVICE_OUT_ALL");
		return 4096;
	}
	else //if(mHandle->devices & AudioSystem::DEVICE_IN_ALL)
	{
//		LOGD("ALSAStreamOps::DEVICE_IN_ALL");
//		return 8192; //soyoung77.park
		size_t bytes = static_cast<size_t>(snd_pcm_frames_to_bytes(mHandle->handle, periodSize));			
                //power of 2.
		for (size_t i = 1; (bytes & ~i) != 0; i<<=1)
		        bytes &= ~i;
		
		LOGI("CAP: ALSA AFbs=%d,PS=%d\n",bytes,periodSize);
	
		return bytes;	//soyoung77.park
	}
#endif
